        ...
        <Table>
          <TableHeader className="sticky top-0 bg-white z-10">
            <TableRow className="bg-gray-50/50 border-b">
              {headers.map(header => (
                <TableHead key={header} className="font-semibold text-gray-700 py-4 px-6 min-w-[150px]">
                  {header.charAt(0).toUpperCase() + header.slice(1).replace(/([A-Z])/g, ' $1')}
                </TableHead>
              ))}
              <TableHead className="w-32 text-center sticky right-0 bg-gray-50/50">Ações</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {data.map((item: ModuleData) => (
              <TableRow key={item.id} className="hover:bg-gray-50/50 transition-colors border-b border-gray-100">
                {headers.map(header => (
                  <TableCell key={header} className="py-4 px-6 min-w-[150px]">
