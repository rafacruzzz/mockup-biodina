...
interface PropostaDTModalProps {
  open: boolean;
  onClose: () => void;
...
const PropostaDTModal = ({ open, onClose, onSave }: PropostaDTModalProps) => {
  ...
  // Representante Legal (editável, pré-preenchido)
  ...
  // Itens da proposta
  const [itens, setItens] = useState<PropostaDTItem[]>([
    { id: '1', descricao: '', referencia: '', marca: '', quantidade: 1, valorUnitario: 0 }
  ]);
  ...
  const addItem = () => {
    setItens([...itens, {
      ...
      descricao: '', referencia: '', marca: '', quantidade: 1, valorUnitario: 0
    }]);
  };
  ...
  const updateItem = (id: string, field: keyof PropostaDTItem, value: string | number) => {
  ...
  const handleSave = () => {
    ...
    toast.success('Proposta DT criada com sucesso!');
    onClose();
  };
            ...
            <CardContent className="space-y-3">
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="min-w-[200px]">Especificação/Descrição</TableHead>
...
export default PropostaDTModal;
