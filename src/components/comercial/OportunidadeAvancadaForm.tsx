...
const OportunidadeAvancadaForm = ({ isOpen, onClose, onSave, oportunidade }: OportunidadeAvancadaFormProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Modalidade
    modalidade: 'oportunidade_avancada'
  });
  ...
  const handleVincularLicitacao = (licitacaoId: string) => {
    ...
    setFormData(prev => ({
      ...
      fluxoTrabalho: `Oportunidade derivada da licitação ${licitacao.numeroPregao}`,
      status: 'ganha',
                    ...
                    {licitacaoVinculadaData && (
                      <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <p className="text-xs text-green-600 mt-1">
                          Os dados do cliente, documentos, histórico e análise técnica foram importados automaticamente.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center space-x-2 mt-4">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          handleInputChange('solicitarAnaliseTecnica', checked);
                          if (checked) {
                            toast({
                              title: "Análise Técnica Solicitada",
                              description: "A equipe técnica foi notificada e realizará a análise do edital em breve.",
                              duration: 4000,
                            });
                          }
                        }}
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Marque esta opção para solicitar uma análise técnica detalhada do edital pela equipe especializada
                    </p>
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        rows={8}
                        className="w-full"
                        disabled={formData.solicitarAnaliseTecnica}
                        readOnly={formData.solicitarAnaliseTecnica}
                      />
                      {formData.solicitarAnaliseTecnica && (
                        <p className="text-xs text-amber-600 mt-1">
                          Este campo será preenchido automaticamente pela equipe técnica
                        </p>
                      )}
                    </div>
                </CardContent>
