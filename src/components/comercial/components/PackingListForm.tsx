...
import { Upload, AlertTriangle, FileUp, Bell, RefreshCw, Clock } from 'lucide-react';
...
const PackingListForm = ({ formData, onInputChange }: PackingListFormProps) => {
  ...
  const [historicoReenvios, setHistoricoReenvios] = useState<Array<{
    id: number;
    data: string;
    hora: string;
    usuario: string;
            ...
            <TabsContent value="historico" className="space-y-6 mt-6">
              <div className="border p-4 rounded bg-blue-50">
                <h3 className="font-bold mb-4 text-lg text-blue-700 flex items-center gap-2">
                  ...
                  Histórico de Reenvios de PACKING List
                </h3>
              </div>

              {historicoReenvios.length === 0 ? (
                <div className="text-center p-8 text-gray-500">
                  <Clock className="h-12 w-12 mx-auto mb-4 text-gray-300" />
                  <p>Nenhum histórico de reenvio encontrado</p>
                </div>
              ) : (
