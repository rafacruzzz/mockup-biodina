...
import { X, Save, Plus, Trash2, ShoppingCart, FileText, Package, Search } from "lucide-react";
...
const NovoPedidoModal = ({ onClose, onSave }: NovoPedidoModalProps) => {
  ...
  const [recebimentoLiberado, setRecebimentoLiberado] = useState(false);

  // Estado para dados de recebimento
  ...
  const [itensRecebimento, setItensRecebimento] = useState<any[]>([]);
  ...
  const handleGerarNF = () => {
    console.log('Gerando NF de entrada:', dadosNF);
    setNfGerada(true);
  };

  const handleConfirmarRecebimento = () => {
    // Inicializar itens de recebimento com base nos produtos
    const itensIniciais = itens.map((item, index) => ({
      id: item.id,
      numero: index + 1,
      quantidade: item.quantidade,
      numeroSerie: '',
      ...
      dataFabricacao: '',
      dataValidade: '',
      localizacao: 'Gal Pria 1',
      lote: '',
      pedidoVenda: ''
    }));
    
    setItensRecebimento(itensIniciais);
    setRecebimentoLiberado(true);
    setActiveTab("recebimento");
  };

  const handleItemRecebimentoChange = (id: number, field: string, value: any) => {
      ...
      <Card className="w-full max-w-7xl max-h-[95vh] overflow-y-auto">
        ...
        <CardContent className="p-0">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className={`grid w-full ${recebimentoLiberado ? 'grid-cols-3' : 'grid-cols-2'} bg-muted/50`}>
            ...
            <TabsContent value="dados-gerais" className="p-6 space-y-6">
              {/* Informações Básicas */}
              <Card>
                <CardHeader>
            ...
            <TabsContent value="processo-importacao" className="p-6 space-y-6">
              {/* Identificação da Nota */}
              <Card>
                <CardHeader>
            ...
            {/* Nova Aba Recebimento */}
            {recebimentoLiberado && (
              <TabsContent value="recebimento" className="p-6 space-y-6">
                <div className="space-y-6">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          <Label>CNPJ de Destino</Label>
                          <Select 
                            value={dadosRecebimento.cnpjDestino} 
                            onValueChange={(value) => setDadosRecebimento({...dadosRecebimento, cnpjDestino: value})}
                          >
                            <SelectTrigger>
                              <SelectValue placeholder="Selecione o CNPJ" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="matriz">Matriz - 12.345.678/0001-90</SelectItem>
                              <SelectItem value="filial1">Filial SP - 12.345.678/0002-71</SelectItem>
                              <SelectItem value="filial2">Filial RJ - 12.345.678/0003-52</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        ...
                        <div>
                          ...
                          <Select 
                            value={dadosRecebimento.filialDestino} 
                            onValueChange={(value) => setDadosRecebimento({...dadosRecebimento, filialDestino: value})}
                          >
                            <SelectTrigger>
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="cd_sp">CD São Paulo</SelectItem>
                              <SelectItem value="cd_rj">CD Rio de Janeiro</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        ...
                        <div>
                          <Label>CFOP</Label>
                          <Input value="3.102" readOnly className="bg-gray-50" />
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label>Conta/Sub Conta</Label>
                          <div className="flex gap-2">
                            <Input value="20" className="w-16" />
                            <Input value="02.12" />
                            <Button variant="outline" size="sm">
                              <Search className="h-4 w-4" />
                            </Button>
                            <span className="text-sm text-gray-600 self-center">Importação</span>
                          </div>
                        </div>

                        <div>
                          <Label>Cod Trib ICMS</Label>
                          <Select defaultValue="51">
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="00">00 - Tributada</SelectItem>
                              <SelectItem value="10">10 - Com redução</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          <Label>C.Custo</Label>
                          <div className="flex gap-2">
                            <Input value="ESTOQUE" readOnly className="bg-gray-50" />
                            <Button variant="outline" size="sm">
                              <Search className="h-4 w-4" />
                            </Button>
                            <span className="text-sm text-gray-600 self-center">ESTOQUE FORNECEDORES</span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          <Label>Nr Embalagens</Label>
                          <div className="flex gap-2">
                            <Input value="1" className="w-16" />
                            <Button variant="outline" size="sm" type="button">Múltiplo</Button>
                            <span className="text-sm text-gray-600 self-center">Qtd por Embalagem: 1,0000</span>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            <Input value="1,8000" />
                            <span className="text-sm text-gray-600 self-center">(Kg)</span>
                          </div>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  {/* Tabela de Itens para Recebimento */}
                  <Card>
                    <CardHeader>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="w-32">Nr. Série</TableHead>
                              <TableHead className="w-32">Depósito</TableHead>
                              <TableHead className="w-32">Dt Fabricação</TableHead>
                              <TableHead className="w-32">Dt de Validade</TableHead>
                              <TableHead className="w-32">Localização</TableHead>
                              <TableHead className="w-24">Ped Venda</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                              ...
                              <TableRow key={item.id}>
                                ...
                                <TableCell>
                                  <Input
                                    type="number"
                                    value={item.quantidade}
                                    onChange={(e) => handleItemRecebimentoChange(item.id, 'quantidade', Number(e.target.value))}
                                    className="w-16"
                                  />
                                </TableCell>
                                <TableCell>
                                  <Input
                                    value={item.numeroSerie}
                                    onChange={(e) => handleItemRecebimentoChange(item.id, 'numeroSerie', e.target.value)}
                                    className="w-32"
                                  />
                                </TableCell>
                                <TableCell>
                                  <Select 
                                    value={item.deposito} 
                                    onValueChange={(value) => handleItemRecebimentoChange(item.id, 'deposito', value)}
                                  >
                                    <SelectTrigger className="w-32">
                                      <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                      <SelectItem value="Dep MG">Dep MG</SelectItem>
                                      <SelectItem value="Dep SP">Dep SP</SelectItem>
                                      <SelectItem value="Dep RJ">Dep RJ</SelectItem>
                                    </SelectContent>
                                  </Select>
                                </TableCell>
                                <TableCell>
                                  <Input
                                    ...
                                    onChange={(e) => handleItemRecebimentoChange(item.id, 'dataFabricacao', e.target.value)}
                                    className="w-32"
                                  />
                                </TableCell>
                                <TableCell>
                                  <Input
                                    type="date" 
                                    value={item.dataValidade}
                                    onChange={(e) => handleItemRecebimentoChange(item.id, 'dataValidade', e.target.value)}
                                    className="w-32"
                                ...
                                <TableCell>
                                  <div className="flex gap-1">
                                    <Input
                                      value={item.localizacao}
                                      onChange={(e) => handleItemRecebimentoChange(item.id, 'localizacao', e.target.value)}
                                      className="w-24"
                                    />
                                    <Button variant="outline" size="sm">
                                ...
                                <TableCell>
                                  <div className="flex gap-1">
                                    <Input
                                      value={item.pedidoVenda}
                                      onChange={(e) => handleItemRecebimentoChange(item.id, 'pedidoVenda', e.target.value)}
                                      className="w-20"
                                    />
                                    <Button variant="outline" size="sm">
                                      <Search className="h-3 w-3" />
