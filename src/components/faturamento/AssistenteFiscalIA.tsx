...
const AssistenteFiscalIA = () => {
  const osPendentesBaixaFiscal = [
    {
      ...
      dataRemessa: new Date(Date.now() - 26 * 60 * 60 * 1000),
  ...
  const alertas: AlertaFiscal[] = [
    { tipo: 'warning', mensagem: '3 NF-e aguardando retorno da SEFAZ.', icon: <Clock className="h-4 w-4" /> },
    { tipo: 'warning', mensagem: '1 NFS-e rejeitada: código de serviço inválido.', icon: <AlertTriangle className="h-4 w-4" /> },
    { tipo: 'suggestion', mensagem: 'Sugestão: revisar alíquota ISS de 2% para município de São Paulo.', icon: <Lightbulb className="h-4 w-4" /> },
    { tipo: 'success', mensagem: '15 NF-e autorizadas hoje.', icon: <CheckCircle className="h-4 w-4" /> }
  ];
  ...
  const getTipoBadge = (tipo: string) => {
    switch (tipo) {
      ...
      case 'suggestion': return 'bg-blue-500/10 text-blue-700 border-blue-500/20';
  ...
  const getTipoLabel = (tipo: string) => {
    switch (tipo) {
      ...
      case 'suggestion': return 'Dica IA';
      default: return 'Info';
    }
  };

  return (
    <Card className="shadow-lg border-imuv-cyan/20">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg flex items-center gap-2 text-imuv-blue">
          <Lightbulb className="h-5 w-5 text-imuv-cyan" />
      ...
      <CardContent className="space-y-3">
        ...
        {alertas.map((alerta, index) => (
          <div key={index} className="p-3 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
            <div className="flex items-start gap-2 mb-2">
              <div className="text-imuv-cyan mt-0.5">{alerta.icon}</div>
              <Badge variant="outline" className={`text-xs ${getTipoBadge(alerta.tipo)}`}>
