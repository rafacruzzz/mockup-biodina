...
import { RegistroAnvisaData, ArquivoAnvisa } from '@/types/anvisaRegistro';

interface DisponibilizacaoInstrucaoStepProps {
  produtoSelecionado: any;
  registroData: RegistroAnvisaData;
  onVoltar: () => void;
  onSalvarRegistro: (data: RegistroAnvisaData) => void;
}

export const DisponibilizacaoInstrucaoStep = ({ 
  produtoSelecionado, 
  registroData, 
  onVoltar, 
  onSalvarRegistro 
}: DisponibilizacaoInstrucaoStepProps) => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      arquivoInstrucaoUso: novoArquivo
    }));
  ...
  const handleSalvarRegistro = () => {
    ...
    onSalvarRegistro(formData);
    toast.success('Registro ANVISA salvo com sucesso!', {
      description: 'O produto foi registrado e está disponível na tabela de registros.'
    });
  };
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-green-800">
                      {formData.arquivoInstrucaoUso.nome}
                    </p>
                    <p className="text-sm text-green-600">
                      {formatarTamanhoArquivo(formData.arquivoInstrucaoUso.tamanho)} • 
                      Carregado em {formData.arquivoInstrucaoUso.dataUpload.toLocaleDateString()}
                    </p>
                  </div>
                </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Resumo do Registro ANVISA</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <span className="font-medium">Nome do Arquivo Principal:</span>
              <p className="text-muted-foreground">{formData.nomeArquivoPrincipal}</p>
            </div>
            <div>
              <span className="font-medium">Área da ANVISA:</span>
              <p className="text-muted-foreground">
                {formData.areaAnvisa === 'produtos_saude' ? 'Produtos para Saúde' : 
                 formData.areaAnvisa === 'diagnostico_in_vitro' ? 'Diagnóstico In Vitro' : 
                 formData.areaAnvisa}
              </p>
            </div>
            <div>
              <span className="font-medium">Nº do Processo:</span>
              <p className="text-muted-foreground">{formData.numeroProcessoAnvisa || 'Não informado'}</p>
            </div>
            <div>
              <span className="font-medium">Pastas de Documentos:</span>
              <p className="text-muted-foreground">{formData.estruturaArquivos?.length || 0} pasta(s)</p>
            </div>
            <div>
              <span className="font-medium">Protocolo Peticionamento:</span>
              <p className="text-muted-foreground">
                {formData.protocoloPeticionamento ? 'Anexado' : 'Não anexado'}
              </p>
            </div>
            <div>
              <span className="font-medium">Instrução de Uso:</span>
              <p className="text-muted-foreground">
                {formData.arquivoInstrucaoUso ? 'Anexado' : 'Não anexado'}
              </p>
            </div>
          </div>
        </CardContent>
