...
import { DocumentoAcaoCampo, SumarioAlertaData } from "@/types/acaoCampo";
import { SumarioAlertaForm } from "./SumarioAlertaForm";
...
export const SumarioAlertaCard = ({ documento, onSave, onGeneratePDF }: Props) => {
  const [showForm, setShowForm] = useState(false);

  const handleSave = (dados: SumarioAlertaData) => {
    onSave(dados);
    setShowForm(false);
  ...
  const dados = documento.dadosSumarioAlerta;
    ...
    <>
              ...
              <div className="p-3 bg-muted rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{dados.titulo || '-'}</p>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Sumário de Alerta</DialogTitle>
          </DialogHeader>
          <SumarioAlertaForm
            initialData={dados}
            onSave={handleSave}
